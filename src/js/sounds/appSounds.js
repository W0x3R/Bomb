export const appSounds = {
	mainSound: document.querySelector('.auido__main'),
	callDevil: document.querySelector('.auido__call-devil'),
	changeLanguage: document.querySelector('.audio__change-language'),
	searchedItem: document.querySelector('.audio__searched-item'),
	shakeBook: document.querySelector('.audio__shake-book'),
	win: document.querySelector('.audio__win'),
	defeat: document.querySelector('.audio__defeat'),
	defeatSecond: document.querySelector('.audio__defeat-second'),
}